...
export default {
  ...
  async execute(interaction, client) {
    ...
    if (!targetMember && ['mute', 'timeout', 'kick', 'ban'].includes(subcommand)) {
    ...
    try {
      switch (subcommand) {
        ...
        case 'mute': {
          const muteDurationStr = interaction.options.getString('duration');
          const muteDuration = parseDuration(muteDurationStr) || 3600000;
          if (targetMember) {
            await targetMember.timeout(muteDuration, reason);
          }
          actionResult = `🔇 Пользователь ${user} замучен на ${muteDurationStr || '1h'}.`;
          break;
        }
        ...
        case 'ban': {
          ...
          const banDurationMs = parseDuration(banDuration);
          const banOptions = { reason, deleteMessageSeconds: 7 * 24 * 60 * 60 };
          await targetMember.ban(banOptions);
          if (banDurationMs > 0) {
            setTimeout(async () => {
              await interaction.guild.members.unban(user.id, `Временный бан истёк (${banDuration})`).catch(() => {});
            }, banDurationMs);
          }
          actionResult = `🔨 Пользователь ${user} забанен${banDuration ? ` на ${banDuration}` : ' навсегда'}.`;
          break;
        }
