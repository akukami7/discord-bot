...
export default {
  ...
  async handleButton(interaction, client) {
    ...
    // Re-check balances
    const ch = await User.findOne({ guildId, userId: duel.challengerId });
    const op = await User.findOne({ guildId, userId: duel.opponentId });
    if (!ch || ch.balance < amount || !op || op.balance < amount) {
      duel.status = 'expired';
      await duel.save();
      return interaction.editReply({ embeds: [new EmbedBuilder().setDescription('❌ У одного из участников недостаточно монет.').setColor(0xED4245)], components: [] });
    }
    ...
    await User.findOneAndUpdate({ guildId, userId: winnerId }, { $inc: { balance: amount } });
    await User.findOneAndUpdate({ guildId, userId: loserId }, { $inc: { balance: -amount } });

    await Transaction.create({ guildId, fromId: loserId, toId: winnerId, amount, type: 'duel', description: `Дуэль: выигрыш` });
